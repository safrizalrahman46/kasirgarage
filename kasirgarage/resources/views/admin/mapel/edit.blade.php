...
<div class="card mb-3">
    ...
    <form action="/mapel/{{ $mapel->id }}/update" method="POST">
        ...
        <div class="form-label">
          ...
          <input type="number" name="kode_mapel" placeholder="kode_mapel" value="{{ $mapel->kode_mapel }}" class="form-control" id="exampleInputEmail1" >
        </div>
            ...
            <div class="form-select mb-2">
                ...
                {{-- @dd($nilais) --}}
                  ...
                  @endforeach --}}
                    @foreach($guru as $gr)
                      <option value="{{ $gr->id }}"  @selected($gr->id == $mapel->guru->id) >{{ $gr->nama }}</option>
                    @endforeach
                </optgroup>
